...
## 📌 v-show<br>  
`v-show`를 사용해서도 요소를 아래와 같이 표현할 수 있다.  
```html  
<h2 v-show="true">Hello Vue3</h2>  
```  
이렇게만 봤을 때는 `v-if`와 사용법은 똑같고 큰 차이가 없는 것으로 보이는데, `v-if`와 `v-show`의 가장 큰 차이는 요소의 렌더링 여부이다.  
...
![IMAGE](https://raw.githubusercontent.com/nogi-bot/resources/main/jeygeon/images/18cd9fb7-778d-40c7-9bbd-72d34df28927-image.png)  
